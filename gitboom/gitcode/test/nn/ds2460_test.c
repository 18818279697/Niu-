...
int main(int argc, char **argv)
{
    ...
    slave_address= strtoul(argv[2], NULL, 0); //写地址
    reg_address= strtoul(argv[3], NULL, 0);   //寄存器开始位置地址
    length= strtoul(argv[4], NULL, 0);
    value= strtoul(argv[5], NULL, 0);         //要写入的值
	rd_wr=slave_address %2;                  //对写地址最后一位处理，如果余数为0，就是写
	slave_address=slave_address/2;           //写地址右移一位 成为设备地址
	...
	if(rd_wr==0){
	    ds2460_date.nmsgs = 1;
		ds2460_date.msgs[0].len = length+1;//
		ds2460_date.msgs[0].addr = slave_address;
		ds2460_date.msgs[0].flags = 0;//写命令
		ds2460_date.msgs[0].buf = (unsigned char*)malloc(length+1);
		ds2460_date.msgs[0].buf[0] = reg_address;//信息值1 寄存器地址，即你要往哪写
		/*for(i=1,j=5;j<argc;j++,i++){
			sscanf(argv[j], "%x", &value);
			ds2460_date.msgs[0].buf[i] = value;//信息值2,即你要写什么
		}*/
		 memset(ds2460_date.msgs[0].buf+1,value,length); //对缓存区后length长度的操作全部赋值value 
	...
	ds2460_date.msgs[0].len = 1;//信息长度为1，第一次只写要读的eeprom中存储单元的地址
	ds2460_date.msgs[0].addr = slave_address; //器件地址
	ds2460_date.msgs[0].flags = 0;//写命令，看读时序理解
	ds2460_date.msgs[0].buf = (unsigned char*)malloc(1);
	ds2460_date.msgs[0].buf[0] = reg_address;//要写入数据的单元地址
